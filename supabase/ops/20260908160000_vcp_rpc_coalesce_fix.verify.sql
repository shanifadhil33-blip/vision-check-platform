...
-- Read the json cell. Verdict must be MIGRATION VERIFIED before continuing.
...
from (
  ...
  bad_grammar_prefix as (
    ...
    where n.nspname in ('public', 'vcp')
      and position(needle.token in p.prosrc) > 0
  ),
  rls_failures as (
    select c.relname
    from pg_class c
  ...
  registered_versions as (
    select m.version
    from vcp.schema_migrations m
    join expected_versions e on e.version = m.version
    where to_regclass('vcp.schema_migrations') is not null
  ),
  missing_versions as (
    select e.version
    from expected_versions e
    where to_regclass('vcp.schema_migrations') is null
       or not exists (
         select 1 from registered_versions r where r.version = e.version
       )
  ),
  proconfig_report as (
    select jsonb_agg(
  ...
  select jsonb_build_object(
    'no_pg_catalog_grammar_prefixes', jsonb_build_object(
      'expected', '[]'::jsonb,
      'observed', coalesce(
        (
          select jsonb_agg(
            ...
            order by b.nspname, b.proname, b.token
          )
          from bad_grammar_prefix b
        ),
        '[]'::jsonb
      ),
      'result', case
        when not exists (select 1 from bad_grammar_prefix) then 'PASS'
        else 'FAIL'
      end
    ),
    ...
    'regression_anon_no_vcp_usage', jsonb_build_object(
      'expected', false,
      'observed', case
    ...
    'all_three_versions_registered', jsonb_build_object(
      'expected', (select jsonb_agg(e.version order by e.version) from expected_versions e),
      'observed', case
      ...
      'missing', coalesce(
        (select jsonb_agg(m.version order by m.version) from missing_versions m),
        '[]'::jsonb
      ),
      'result', case
        when not exists (select 1 from missing_versions) then 'PASS'
        else 'FAIL'
      end
    ),
